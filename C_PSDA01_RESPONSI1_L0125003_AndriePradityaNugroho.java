...
class BinarySearchTree {
    ...
    Node insert(Node root, String name) {
        ...
        if (
            name.compareTo(root.name) < 0 // Comparing the value of the strings by each char
    ...
    Node search(Node root, String name) {
        if (root == null || name.equals(root.name)) return root;
    ...
    Node deleteNode(Node root, String name) {
        ...
        if (name.compareTo(root.name) < 0) root.left = deleteNode(
            root.left,
            name
        );
        else if (name.compareTo(root.name) > 0) root.right = deleteNode(
            root.right,
            name
        );
        else {
    ...
    public static void main(String[] args) {
        ...
        while (true) {
            ...
            if (input.equals("EXIT")) {
                ...
                System.out.println(
                    "ADD: Input a new contact if not exist yet."
                );
                System.out.println(
                    "DELETE: Delete an existing contact with given name."
                );
                System.out.println(
                    "UPDATE: Update the phone number of specified contact."
                );
                System.out.println(
                    "GET: Print the contact's name with the phone number."
                );
                System.out.println(
                ...
                if (log.isEmpty()) {
                    System.out.println("No logs has been recorded.");
                }
                ...
                switch (parts[0].toUpperCase()) {
                    ...
                    case "DELETE":
                        ...
                        log.addLast("[" + time + "] " + "DELETE: " + parts[1]);
